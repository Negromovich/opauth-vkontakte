...
class VKontakteStrategy extends OpauthStrategy
{
    ...
    public function int_callback()
    {
        if (array_key_exists('code', $_GET) && !empty($_GET['code'])) {
            $url
                = 'https://oauth.vk.com/access_token'; //DGB 2012-11-06 Notice VK documentation is wrong, because they DO require HTTPS
            $params = array(
                'client_id'     => $this->strategy['app_id'],
            ...
            $vkuser = $this->getuser($results['access_token'], $results['user_id']);
            ...
            if (!empty($vkuser['first_name'])) {
                ...
                $this->auth['info']['first_name'] = $vkuser['first_name'];
            }
            if (!empty($vkuser['last_name'])) {
                $this->auth['info']['name'] = $vkuser['last_name'];
                $this->auth['info']['last_name'] = $vkuser['last_name'];
            }
            if (!empty($vkuser['first_name']) && !empty($vkuser['last_name'])) {
                $this->auth['info']['name'] = $vkuser['first_name'] . ' ' . $vkuser['last_name'];
            }

            if (!empty($vkuser['nickname'])) {
                $this->auth['info']['nickname'] = $vkuser['nickname'];
            } elseif (!empty($vkuser['screen_name'])) {
            ...
            } elseif (!empty($vkuser['photo_medium'])) {
                $this->auth['info']['image'] = $vkuser['photo_medium'];
            } elseif (!empty($vkuser['photo'])) {
                $this->auth['info']['image'] = $vkuser['photo'];
            }

            if (!empty($vkuser['birthday'])) {
                $this->auth['info']['bdate'] = date('c', strtotime($vkuser['birthday']));
            }
    ...
    private function getuser($access_token, $uid)
    {
        ...
        $vkuser = $this->serverGet('https://api.vk.com/method/users.get', array('access_token' => $access_token, 'uid' => $uid, 'fields' => $fields), null, $headers);
        if (!empty($vkuser)) {
